...
static counter_t one_cycle_stall_q1;
static counter_t two_cycle_stall_q1;

static counter_t one_cycle_stall_q2;
static counter_t two_cycle_stall_q2;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   ...
		   "(one_cycle_stall_q1+two_cycle_stall_q1*2+sim_num_insn)/sim_num_insn", NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "(one_cycle_stall_q2+two_cycle_stall_q2*2+sim_num_insn)/sim_num_insn", NULL);
...
/* ECE552 Assignment 1 - BEGIN CODE */
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        {
            // get the most stalls in one instruction
            if (reg_ready_q1[r_in[i]] - sim_num_insn > mostStalls_q1)
            {
                mostStalls_q1 = reg_ready_q1[r_in[i]] - sim_num_insn;
                // set back the reg ready flag for the registers that has been stall
                reg_ready_q1[r_in[i]] = sim_num_insn;
            }
        }
    }

    // check if there's stall
    if (mostStalls_q1 > 0)
    {
        // RAW hazard detected
        sim_num_RAW_hazard_q1++;

        // update stalls counters
        switch (mostStalls_q1){
            case 1:
                one_cycle_stall_q1++;
                break;
            case 2:
    ...
    // update the output register ready cycle
    // F D EX M WB
    //   F S  S D  EX M WB
    if (r_out[0] != DNA)
    ...
    for (k = 0; k < 3; k++)
    {
        ...
        {
            // if there's store operation, skip
            // no need to stall for store operation
            ...
            // get the most stalls in one instruction
            if (reg_ready_q2[r_in[k]] - sim_num_insn > mostStalls_q2)
            {
                mostStalls_q2 = reg_ready_q2[r_in[k]] - sim_num_insn;
                // set back the reg ready flag for the registers that has been stall
                reg_ready_q2[r_in[i]] = sim_num_insn;
            }
        }
    }
    ...
    // update the output register ready cycle for store case
    // no need to stall for store case
    // F D EX1 EX2 M   WB
    //   F D   EX1 EX2 M  WB
    if ((MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_STORE))
    {
        if (r_out[0] != DNA)
            reg_ready_q2[r_out[0]] = sim_num_insn + 1;
        if (r_out[1] != DNA)
            reg_ready_q2[r_out[1]] = sim_num_insn + 1;
    }

    // update the output register ready cycle for load case
    // load operation result is done at M-stage instead of EX2 stage
    // forwarding happens in the cycle after M
    // F D EX1 EX2 M WB
    //   F D   S   S EX1 EX2 M WB
    ...
    // update the output register ready cycle for rest cases (sub, add...)
    // result from previous EX2 will be forwarding to the EX1 stage of next instruction
    // F D EX1 EX2 M WB
    //   F D   S   EX1 EX2 M WB
    else
    {
